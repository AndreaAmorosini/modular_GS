...
def filter_ply_by_opacity(
    ...
    try:
        ...
        logger.warning(f"[Core] Error reading PLY: {e}")
        return

    if "vertex" not in plydata:
        logger.error("[Core] Error: 'vertex' element not found in PLY.")
        return

    vertex = plydata["vertex"]

    if "opacity" not in vertex.data.dtype.names:
        logger.error("[Core] Error: 'opacity' property not found in vertex data.")
        return
    ...
    logger.debug(f"Filtered PLY saved to: {output_path}")
